#!/bin/bash
## used for preprocessing reads without stagger sequences
...
#zcat $f | perl $HOME/perl_script/preprocessReads_v3.pl 5 - 2> >(gzip > ${bf}_unmatched.fastq.gz) | gzip - > ${bf}_preProcessed.fastq.gz
...
perl $HOME/perl_script/countReads_perGuide.pl ${bf}_readCount.tab T > ${bf}_geneCount.tab
perl $HOME/perl_script/countReads_perGuide.pl ${bf}_readCount.tab F > ${bf}_geneRawCount.tab
